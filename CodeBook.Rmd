...
names(y_test) <- "key"
y_test_labels <- merge(y_test, activity_labels[, c("key", "activity")], by = "key", all.x = TRUE)

# Merge train data with labels
names(y_train) <- "key"
y_train_labels <- merge(y_train, activity_labels[, c("key", "activity")], by = "key", all.x = TRUE)

# Combine test and train data labels, TEST DATA FIRST
merged_activity <- rbind(y_test_labels, y_train_labels)
```
...
```{r}
merged_df_all <- cbind(merged_df, merged_activity, subject = merged_subject)
```
